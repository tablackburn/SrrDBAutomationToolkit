...
- `Get-SatReleaseFile` no longer attempts to download files that srrDB doesn't host (RAR archives, sample videos, etc.), eliminating 404 warnings
  - Added whitelist filter for downloadable file types: `.nfo`, `.sfv`, `.txt`, `.jpg`, `.jpeg`, `.png`
  - Improved handling of null and whitespace-only filenames
...
- `Get-SatFile` - Download stored files (proof images, NFO, SFV, etc.) from srrDB
  - Supports pipeline input with `ReleaseName` and `FileName` properties
  - Optional `OutPath` parameter, defaults to current directory
  - `-PassThru` switch to return FileInfo object
...
[0.4.1]: https://github.com/tablackburn/SrrDBAutomationToolkit/compare/v0.4.0...v0.4.1
